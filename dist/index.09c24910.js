...
function animationIndividualSkills(animation, time) {
    setInterval(()=>animation.style.animation = "wiggle 1.5s ease-in-out", time);
}
function animationDobleSkills(animation, time) {
    setInterval(function() {
        Array.from(animation).forEach((a)=>a.style.animation = "wiggle 1.5s ease-in-out");
    }, time);
}
...
const btnsCloseModal = document.querySelectorAll(".project-modal--button-close");
...
btnsCloseModal.forEach(function(el) {
    el.addEventListener("click", function(e) {
        ...
        const overlay = modal.closest(".modal").querySelector(".project-modal--overlay");
        closeModal(modal);
